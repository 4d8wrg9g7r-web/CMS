...
The import understands these built-in person fields: firstName, middleName, lastName, suffix, preferredName, email, phone, dateOfBirth, gender, familyPosition, membershipStatus, tags, campus. familyPosition is the person's place in their family (father, mother, head of household, child, grandparent…) — map columns like "Household Role" or "Family Position" to it rather than to a custom field; the same goes for birth-date columns (dateOfBirth) and preferred/nickname columns (preferredName). A single combined name column maps to "fullName" (set nameOrder to how names are written in the samples). A column that groups people into families/households (e.g. "Household", "Family Name") maps to "household". A column that is clearly person data but fits no built-in field (e.g. "Veteran", "Baptism Date", "Ministry Team", "T-Shirt Size") maps to "custom" with a customField: a clean human label and a storage type — BOOLEAN for yes/no shapes, DATE for dates, NUMBER for numbers, SELECT for a small repeated vocabulary, TEXT otherwise. Only columns that mean nothing for a person profile (row numbers, export timestamps, internal ids) map to "ignore".
